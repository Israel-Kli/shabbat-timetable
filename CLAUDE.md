...
- `progress.html` — standalone A4 tracker for the מבצע תפילה בציבור; reuses `styles.css` page chrome, data lives in the `OCCASIONS` array inside the file (append one entry per Shabbat/Yom Tov, totals and ranking are derived; Yom Tov entries take an extra `label` for the update line)
